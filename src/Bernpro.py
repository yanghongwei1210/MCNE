...
class Bern_prop(MessagePassing):
    ...
    def forward(self, x, edge_index,edge_weight=None):
        ...
        # G = nx.Graph()
        # # 添加边，并指定边权
        # G.add_weighted_edges_from(zip(edge_index[0].detach().numpy(), edge_index[1].detach().numpy(), edge_weight.detach().numpy()))
        # L = nx.normalized_laplacian_matrix(G)
        # L=torch.tensor(L.todense(),requires_grad=True)
        # edge_index1, norm1 = dense_to_sparse(L)
