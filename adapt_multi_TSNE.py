...
def cal_acc_multi(A,features,labels, netF_list, netC_list, netG_list, args):
    ...
    with torch.no_grad():
        ...
        all_output = outputs_all_w.float().cpu()
    X_tsne = TSNE(n_components=2,random_state=33).fit_transform(all_output.cpu().detach().numpy())
    plt.figure(figsize=(10, 10))
    plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=labels,label="t-SNE")
    plt.legend()
    plt.savefig(r'picture/Epoch_{}.jpg'.format(args.iter_num),bbox_inches='tight',dpi=1200 )
    plt.show()
    print('----------------')
    _, predict = torch.max(all_output, 1)
